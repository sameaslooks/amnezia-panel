...
class AWGManager:
    ...
    def add_client(self, name: str, server_id: int = None) -> Dict:
        """Добавляет клиента (локально или на удалённом сервере)"""
        
        if not server_id or server_id == 1:  # локальный сервер (id=1)
            return self._add_client_local(name)
        else:
            # Для асинхронного вызова нужно будет обработать иначе
            import asyncio
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            return loop.run_until_complete(
                self.add_client_remote(name, server_id)
            )

    def _add_client_local(self, name: str) -> Dict:
...
"""
    ...
    async def add_client_remote(self, name: str, server_id: int) -> Dict:
        """Добавляет клиента на удалённом сервере"""
        ...
        result = await ssh_manager.execute_command(
            server,
            "docker exec amnezia-awg2 wg genpsk"
        )
        if not result["success"]:
            raise Exception(f"Failed to generate PSK: {result['stderr']}")
        psk = result["stdout"].strip()
        
        # 4. Получаем текущий конфиг
        result = await ssh_manager.execute_command(
            server,
            "docker exec amnezia-awg2 cat /opt/amnezia/awg/awg0.conf"
        )
        if not result["success"]:
            raise Exception(f"Failed to read config: {result['stderr']}")
        config = result["stdout"]
        
        # 5. Определяем следующий IP
        next_ip = self._get_next_ip(config)
        
        # 6. Добавляем секцию пира
        peer_section = f"""
    [Peer]
    PublicKey = {public_key}
    PresharedKey = {psk}
    ...
    """
        ...
        # 7. Записываем новый конфиг
        result = await ssh_manager.execute_command(
            server,
            "docker exec -i amnezia-awg2 tee /opt/amnezia/awg/awg0.conf > /dev/null",
            input_data=new_config
        )
        if not result["success"]:
            raise Exception(f"Failed to write config: {result['stderr']}")
        
        # 8. Синхронизируем конфиг
        result = await ssh_manager.execute_command(
            server,
            "docker exec amnezia-awg2 awg syncconf awg0 <(cat /opt/amnezia/awg/awg0.conf)"
        )
        if not result["success"]:
            raise Exception(f"Failed to sync config: {result['stderr']}")
        
        # 9. Обновляем clientsTable на удалённом сервере
        table_result = await ssh_manager.execute_command(
            server,
            "docker exec amnezia-awg2 cat /opt/amnezia/awg/clientsTable 2>/dev/null || echo '[]'"
        )
        
        try:
            clients_table = json.loads(table_result["stdout"]) if table_result["success"] else []
        except:
            clients_table = []
        ...
        await ssh_manager.execute_command(
            server,
            f"docker exec -i amnezia-awg2 tee /opt/amnezia/awg/clientsTable > /dev/null",
            input_data=json.dumps(clients_table, indent=4)
        )
        
        # 10. Сохраняем в локальную БД
        from database import create_client
        create_client(public_key, name, next_ip, private_key, server_id)
        
        # 11. Сохраняем конфиг клиента в контейнере
        client_config = self.get_client_config_remote(public_key, server)
        
        await ssh_manager.execute_command(
            server,
            f"docker exec -i amnezia-awg2 tee /opt/amnezia/awg/client_configs/{public_key}.conf > /dev/null",
            input_data=client_config
        )
    ...
    def get_client_config_remote(self, public_key: str, server: dict) -> str:
        """Генерирует конфиг для клиента на удалённом сервере"""
        # Получаем параметры сервера
        config = ssh_manager.execute_command(
            server,
            "docker exec amnezia-awg2 cat /opt/amnezia/awg/awg0.conf"
        )["stdout"]
        
        # Ищем IP клиента и PSK
        peers = config.split('[Peer]')[1:]
        client_ip = None
        psk = None
        ...
        # Получаем приватный ключ из локальной БД
        from database import get_client
        client_data = get_client(public_key)
        private_key = client_data.get('private_key', '') if client_data else ''
        ...
        h4 = find(r'H4\s*=\s*(\S+)', '')
        i1 = find(r'I1\s*=\s*(.*)', '')
        listen_port = find(r'ListenPort\s*=\s*(\d+)', '32308')
        ...
        if priv_match:
            server_private = priv_match.group(1)
            server_public = ssh_manager.execute_command(
                server,
                f"echo '{server_private}' | docker exec -i amnezia-awg2 awg pubkey"
            )["stdout"].strip()
